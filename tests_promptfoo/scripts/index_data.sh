...
if [ -n "${INDEX_SCHEMA_JSON_FILE_PATH}" ]; then
    ...
    curl -s -X PUT --insecure -u "$OS_USER":"$OS_PW" "https://localhost:9200/${INDEX_NAME}" -H 'Content-Type: application/json' --data-binary @"${INDEX_SCHEMA_JSON_FILE_PATH}"
    echo -e "\n"
fi

echo -e "${MAJOR}Indexing the data, please wait...\n${RESET}"
# Define the OpenSearch endpoint and content header
OPENSEARCH_URL="https://localhost:9200/${INDEX_NAME}/_bulk?pretty=false&filter_path=-items"
